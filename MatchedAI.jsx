...
export default function MatchedAI() {
  useEffect(() => {
    document.title = 'AI Matched Jobs – jobhunter.ai'
  }, [])
  ...
  const [filterAppliedButNoResults, setFilterAppliedButNoResults] =
    useState(false)
    ...
    async (opts = {}) => {
      ...
      try {
        ...
        if (data) {
          ...
          const raw = data._raw || {}
          setFilterAppliedButNoResults(
            Boolean(raw.filter_applied_but_no_results)
          )
  ...
  useEffect(() => {
    fetchJobs({ query: searchQuery, page })
  ...
  const handleGenerateCoverLetter = async (job) => {
    ...
    try {
      ...
      setCoverModalError(
        String(err?.message || err || 'Failed to generate cover letter')
      )
    } finally {
  ...
  const filterJobs = useMemo(
    () =>
      jobs.filter((j) => {
        ...
        const passExp =
          !experience ||
          jobExp.toLowerCase() === experience.toLowerCase() ||
          jobExp.toLowerCase() === 'unknown' ||
          jobExp === ''
    ...
    [jobs, experience, salaryMin, salaryMax]
  )
  ...
  const handleDownloadTxt = () => {
    ...
    const safeCompany = (selectedJob?.company || 'Company').replace(
      /[^\w.-]+/g,
      '_'
    )
      ...
      <div className="app-row">
        <nav className="sidebar">
          <ul>
            ...
            <li>
              <Link to="/resume_upload">Upload Resume</Link>
            </li>
            <li>
              <Link to="/matchedAI">AI Matched Jobs</Link>
            </li>
            <li>
        ...
        <main className="container">
          ...
          <p className="text-muted-foreground">
            Jobs matched to your resume and preferences.
          </p>
          <p className="text-muted-foreground">
            Use filters to help narrow results.
          </p>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 13, marginTop: 4 }}>
                  We couldn&apos;t find jobs that matched your filters exactly.
                  There {unfilteredTotalResults === 1 ? 'is' : 'are'}{' '}
                  {unfilteredTotalResults ?? 'some'} job
                  {unfilteredTotalResults === 1 ? '' : 's'} without the selected
                  filters.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  onClick={() => {
                    ...
                    setExp('')
                    setPage(1)
                    setFilterAppliedButNoResults(false)
                    fetchJobs({
                      query: searchQuery,
                      location,
                      page: 1,
                ...
                >
                  Remove Type &amp; Experience Filters
                </button>
                <button
                  onClick={() => {
                    setType('')
                ...
                >
                  Show {unfilteredTotalResults ?? 'All'} Jobs
                </button>
              </div>
            </div>
          ) : null}
          ...
          {(() => {
            ...
            const hasServerTotal =
              Boolean(totalResults && Number(totalResults) > 0)
            const pagesFromServer = Math.max(
              1,
              Math.ceil(
                (hasServerTotal ? Number(totalResults) : jobs.length || 0) /
                  RESULTS_PER_PAGE
              )
            )
            const pagesFromFiltered = Math.max(
              1,
              Math.ceil(filteredCount / RESULTS_PER_PAGE)
            )
            const totalPages = hasServerTotal
              ? pagesFromServer
              : pagesFromFiltered
          ...
          {jobModalOpen ? (
            <div
              ...
              onClick={() => {
                if (!jobActionLoading) closeJobModal()
              }}
            >
              <div
              ...
              >
                ...
                <div style={{ marginTop: 12 }}>
                  <div style={{ color: 'var(--muted)' }}>
                    ...
                    {selectedJob?.type ? `• ${selectedJob?.type}` : ''}
                  </div>
                  <div style={{ marginTop: 12 }}>
                  ...
                  {jobModalMessage ? (
                    <div
                      style={{
                        marginTop: 8,
                        color: jobModalMessage.startsWith('Failed')
                          ? 'crimson'
                          : 'green',
                      }}
                    >
          ...
          {coverModalOpen ? (
            <div
              ...
              onClick={() => {
                if (!coverModalLoading) setCoverModalOpen(false)
              }}
            >
              <div
                className="cover-modal"
    ...
    >
      <div data-slot="card-header" style={{ textAlign: 'left' }}>
        ...
        >
          <div style={{ minWidth: 0 }}>
            <h4 data-slot="card-title" style={{ margin: 0 }}>
              {job.title}
            </h4>
      ...
      <div data-slot="card-content">
        ...
        >
          ...
          <button onClick={() => onGenerateCover && onGenerateCover()}>
            Cover Letter
          </button>
        </div>
      </div>
